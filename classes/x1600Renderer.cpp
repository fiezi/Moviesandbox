...
void X1600Renderer::drawSceneTexture(){
    ...
    for (int i=0;i<(int)layerList.size();i++){
        ...
        glBlitFramebufferEXT( 0, 0, scene_size, scene_size, 0, 0, scene_size, scene_size, GL_COLOR_BUFFER_BIT, GL_LINEAR );
...
void X1600Renderer::drawData3D(Layer* currentLayer){
    ...
    if (!input->controller->bRunning){
        ...
        for (int i=0;i<(int)helperList.size();i++){
            if (!helperList[i]->bHidden && helperList[i]->bPickable){
				drawActor(helperList[i]);
            }
        }
