...
void setup() {
   ...
   */
  myRemoteLocation = new NetAddress("192.168.0.2",31848);
...
void serialEvent (Serial myPort) {
 ...
 println("serial!");
  ...
  if (inString != null) {
    ...
    inByte1 = map(inByte1, 30, 120, 0, 1.0); 
    inByte1 = max(0,inByte1);
    inByte1 = min(1.0,inByte1);
   }
  ...
  if (inString != null) {
    ...
    inByte2 = map(inByte2, 30, 120, 0, 1.0); 
    inByte2 = max(0,inByte2);
    inByte2 = min(1.0,inByte2);
   }
  inString = myPort.readStringUntil(';');
  if (inString != null) {
    // trim off any whitespace:
...
//  OscMessage myMessage = new OscMessage("/pilot/vector3f/vector3f");
  ...
  if (bUseArduino){
    ...
    myMessage.add((inByte1) * 100.0); /* add vector z component to the osc message */

    myMessage.add(0.0); /* add vector x component to the osc message */
    myMessage.add(0.0); /* add vector z component to the osc message */
    myMessage.add((inByte2-0.3) * 100.0); /* add vector z component to the osc message */

    if (inByte3<1.0){
      OscMessage triggerMessage= new OscMessage ("/trigger");
      triggerMessage.add("switchCamOne");
      oscP5.send(triggerMessage, myRemoteLocation); 
      //println("switchCamOne!");
    }
    if (inByte4<1.0){
      OscMessage triggerMessage= new OscMessage ("/trigger");
      triggerMessage.add("switchCamTwo");
      oscP5.send(triggerMessage, myRemoteLocation); 
      //println("switchCamTwo!");
    }
    if (inByte5<1.0){
      OscMessage triggerMessage= new OscMessage ("/trigger");
      triggerMessage.add("switchCamThree");
      oscP5.send(triggerMessage, myRemoteLocation); 
      //println("switchCamThree!");
    }
  }
  else{
