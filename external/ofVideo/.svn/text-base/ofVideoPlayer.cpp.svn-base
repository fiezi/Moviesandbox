...
//	short 		actualResId 	= DoTheRightThing;
		...
		if (result != noErr) {
		    cout << "NativePathNameToFSSpec failed " << result << endl;
...
//			ofLog(OF_LOG_ERROR,"Error loading movie");
	...
	if (result == noErr) {
		...
		if (result == noErr){
		    ...
		    cout << "NewMovieFromFile failed " << result << endl;
//			ofLog(OF_LOG_ERROR,"NewMovieFromFile failed %d", result);
			return false;
		}
	} else {
		cout << "OpenMovieFile failed " << result << endl;
...
bool createMovieFromURL(string urlIn,  Movie &movie){
	...
	if ( ( err = MemError()) != noErr){
	    ...
	    cout << "createMovieFromURL: error creating url handle" << endl;
	    return false;}
	...
	if(err != noErr){
		cout << "createMovieFromURL: error loading url" << endl;
