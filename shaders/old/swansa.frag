...
//ambient Light
  ...
  gl_FragData[0].rgb=clamp(gl_FragData[0].rgb, vec3(0.0,0.0,0.0), gl_Color.rgb);
...
//post processing - saturation
  vec4 greyColor= vec4( (gl_FragData[0].r+gl_FragData[0].g+gl_FragData[0].b)/3.0);
  
  greyColor.b+=0.025;
  greyColor.a=1.0;
  gl_FragData[0]= (postColor.a * gl_FragData[0] + (1.0-postColor.a) * greyColor);
